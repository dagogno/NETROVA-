...
class Pack(models.Model):
    ...
    def lien_whatsapp(self):
        """Lien WhatsApp paiement à crédit (acompte 50%)."""
        numero = getattr(settings, 'NETROVA_WHATSAPP', '22890491287')
        message = f"Bonjour NETROVA, je souhaite commander le Pack {self.code} à {self.prix_formate} FCFA (paiement à crédit — acompte 50%)."
        return f"https://wa.me/{numero}?text={quote(message)}"

    def lien_whatsapp_comptant(self):
        """Lien WhatsApp paiement comptant (réduction 2%, livraison offerte)."""
        numero = getattr(settings, 'NETROVA_WHATSAPP', '22890491287')
        reduction = int(self.prix * 0.02)
        prix_reduit = self.prix - reduction
        prix_fmt = f"{prix_reduit:,}".replace(",", " ")
        message = (
            f"Bonjour NETROVA, je souhaite commander le Pack {self.code} "
            f"en PAIEMENT COMPTANT — soit {prix_fmt} FCFA (réduction 2% appliquée). "
            f"Je paie tout d'un coup, pas d'acompte."
        )
        return f"https://wa.me/{numero}?text={quote(message)}"

    @property
    def prix_comptant(self):
        """Prix avec réduction 2% pour paiement comptant."""
        return self.prix - int(self.prix * 0.02)

    @property
    def prix_comptant_formate(self):
        return f"{self.prix_comptant:,}".replace(",", " ")
