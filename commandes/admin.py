...
class CommandeAdmin(admin.ModelAdmin):
    list_display  = ['ref', 'membre_nom', 'pack', 'mode_badge', 'montant_fmt',
    ...
    fieldsets = (
        ('📦 Commande', {
            'fields': ('membre', 'pack', 'quantite', 'mode_commande', 'statut', 'date_commande', 'date_limite_solde', 'note')
        }),
        ('💰 Finances', {
    ...
    def mode_badge(self, obj):
        if obj.mode_commande == 'cash':
            return format_html('<span style="background:#16914E;color:#fff;padding:2px 8px;border-radius:4px;font-size:11px">💵 Comptant</span>')
        return format_html('<span style="background:#1A4DB5;color:#fff;padding:2px 8px;border-radius:4px;font-size:11px">💳 Crédit</span>')
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.mode_commande == 'credit' and obj.membre.kyc_statut != 'valide':
            self.message_user(
                request,
                f"⚠️ {obj.membre.nom_complet} n'a pas de KYC validé pour le programme crédit "
                f"(statut actuel : {obj.membre.get_kyc_statut_display()}). "
                f"La commande a bien été enregistrée à crédit, mais vérifiez son dossier avant de livrer.",
                messages.WARNING,
            )
