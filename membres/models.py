...
class Membre(models.Model):
    ...
    kyc_statut             = models.CharField(max_length=12, choices=KYC_CHOICES, default='non_requis', verbose_name='Statut KYC')
    ...
    @property
    def nb_commandes_soldees(self):
        return self.commandes.filter(statut='soldee').count()

    @property
    def kyc_documents_fournis(self):
        return bool(self.photo_profil and self.carte_identite_recto)

    @property
    def peut_commander_credit(self):
        """True si le KYC est validé : le membre peut participer au programme crédit (paiement par tranches)."""
        return self.kyc_statut == 'valide'

    @property
    def programme_credit_en_attente(self):
        """True si une demande de participation au programme crédit est en cours de vérification."""
        return self.kyc_statut == 'en_attente'
