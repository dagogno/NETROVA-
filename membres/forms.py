...
class InscriptionForm(forms.Form):
    ...
    def save(self):
        ...
        membre = Membre.objects.create(
            ...
            # Pas de KYC à l'inscription : kyc_statut reste 'non_requis' (valeur par défaut du modèle)
            # jusqu'à ce que le membre demande à rejoindre le programme crédit.
        )
        return membre
...
class ProgrammeCreditForm(forms.ModelForm):
    """
    Formulaire de participation au programme crédit (paiement par tranches).
    C'est le SEUL endroit où le KYC est demandé : un membre n'y accède que
    s'il en fait la démarche volontairement, jamais à l'inscription.
    """
    class Meta:
        model  = Membre
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['photo_profil'].label = 'Photo de profil * (selfie)'
        self.fields['carte_identite_recto'].required = True
        self.fields['carte_identite_recto'].label = 'CNI / Passeport — Recto *'
        self.fields['carte_identite_verso'].required = False
        self.fields['carte_identite_verso'].label = 'CNI — Verso (optionnel)'
